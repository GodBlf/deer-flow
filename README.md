...
In **Settings > Memory**, DeerMem users can select the default assistant or a custom
agent to search, add, edit, delete, or clear that agent's facts. The selector shows
fact counts and also exposes remaining facts whose agent configuration is missing.
User profile and history summaries are shared across agents and appear once.
**Clear selected agent facts** preserves those summaries and all other agents;
**Clear all memory** removes every fact bucket and the shared summaries for the
current user. Clearing cancels pending extraction for the affected scope; work
already executing is not interrupted, and later conversations may create new facts.

Import/export keeps its existing scope: **shared summaries and default assistant
facts**, regardless of the selector. Import replaces those shared summaries and
default facts; the export is not an all-agent backup. Alternative memory backends
retain their existing management behavior; scoped controls require an explicit
backend capability. No new data migration is required for this browser.
